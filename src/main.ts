...
class Network {
  ...
  addHop(name: string, probeId: string): void {
    if (this.nodes[name].outgoing.length > 0) {
      ...
      if (
        this.probes[probeId].nodes.indexOf(this.nodes[name].outgoing[0]) === -1
      ) {
  ...
  async runTasks(): Promise<void> {
    while (Object.keys(this.probes).length > 0) {
      for (
        let i = 0;
        i < Object.keys(this.probes).length && i < CHUNK_SIZE;
        i++
      ) {
      ...
      console.log(
        `Another ${CHUNK_SIZE} tasks done of ${Object.keys(this.probes).length}`,
      );
