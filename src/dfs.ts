...
const SOLUTIONCSV = './solution.csv';
console.log('Opening', DEBTCSV);
const check =
  '19897 23610 21257 20069 22568 27857 27867 27868 27869 27865 27872 21250 21549 9410 21925 22969 22100 22112 21523 24137 20484 22590 22103 21750 27561 21770 21811 29414 19882 20467 24126 20493 22591 22102 21768 27560 24105 22579 22105 21745 27562 21429 21822 29406 19855 22578 22110 21520 27564 21420 21833 19585 21851 22569 22107 21743 27563 21426 21823 23620 21253 21280 33361 31417 31376 31420 31382 20475 19939 29405 19897'.split(
    ' ',
  ); // weight 45, length 71
const linksCheck = {};
...
lineReader.on('line', function (line) {
  ...
  if (typeof linksCheck[from][to] === 'undefined') {
...
lineReader.on('close', async function () {
  ...
  for (let i = 0; i < check.length - 1; i++) {
    if (typeof linksCheck[check[i]] === 'undefined') {
      throw new Error(`no way out of ${check[i]}`);
    }
    if (typeof linksCheck[check[i]][check[i + 1]] === 'undefined') {
      throw new Error(`no way from ${check[i]} to  ${check[i + 1]}`);
    }
    if (linksCheck[check[i]][check[i + 1]] < 114) {
      throw new Error(
        `link from ${check[i]} to ${check[i + 1]} is only ${linksCheck[check[i]][check[i + 1]]} wide`,
      );
    }
  ...
  console.log(
    `Graph has ${Object.keys(links).length} nodes and ${numLinks} links left`,
  );
  console.log(
    `After ${numTrans} transactions with a total amount of ${Math.round(totalTransAmount / 1000000)} million`,
  );
  let totalNum = 0;
  ...
  console.log(
    `${Math.round(totalMultilateral / 1000000)} million (${Math.round((totalMultilateral / totalTransAmount) * 100)}%) was netted in ${totalNum} loops`,
  );
  console.log(
    `Leaving ${Math.round(amountLeft / 1000000)} million (${Math.round((amountLeft / totalTransAmount) * 100)}%) to be settled out of band`,
  );
});
